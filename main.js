...
const commands = ["RAS", "ECHO", "PIXIV", "SYSTEM", "PEAKY P-KEY"];

function open_desktop(element) {
...
function log(str) {
    ...
    $('#tty').append("[    OK    ] " + time.toString().padStart(10, "0") + " " + str + "<br />");
}

$(document).ready(function () {
    $("#sys-info").append("授權金鑰：<br/>" + btoa("HAHA, it is just your user agent: " + navigator.userAgent));
    log("Authorization key: " + btoa("HAHA, it is just your user agent: " + navigator.userAgent))
    log("Welcome to mizu system")
    // device detection
    ...
    log("Loaded module live2d")
    $('.card').css("position", "fixed");
    $('.card').hide();
    //boot
    $.get("https://api.github.com/search/users?q=MizuyoruTW", (result) => {
...
function summit_command() {
    ...
    switch (index) {
        ...
        case 3://system
            ...
            break;
        case 4:
            window.open("https://d4dj-pj.com/character/peakyp-key/");
            break;
        default:
            $('#login_form_command').addClass("is-invalid");
            $('#login_form_command').effect("shake");
            log("[Termizu] Invalid command: " + $("#login_form_command").val())
    }
    if (index > -1) {
